...
identity_public_endpoint = endpoint "identity-public"
...
db_user = node["identity"]["db"]["user"]
#get password from attribs as encrypted databag doesn't yet exist - JPA
#db_pass = db_password "keystone"
db_pass = node["identity"]["db"]["password"]
...
uris = {
  ...
  'identity-public' => identity_public_endpoint.to_s.gsub('%25','%'), 
...
pe = identity_public_endpoint
public_endpoint = "#{pe.scheme}://#{pe.host}:#{pe.port}/"
...
# set up the keystone user, and database
template "/tmp/keystone_grants.sql" do
  source "keystone_grants.sql.erb"
  mode "0644"
end

execute "setup_keystonedb" do
  mysql_pwd = node["mysql"]["root_password"]
  setup_command = "mysql --user=root --password=#{mysql_pwd} < /tmp/keystone_grants.sql"
